...
export default function DistributorAsCustomerPage() {
  ...
  const reminderMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.rpc("send_order_reminder", toSendOrderReminderRpcArgs(input));
      if (error) throw error;
    },
    onSuccess: () => {
      showToast("התזכורת נשלחה.", "success");
      void queryClient.invalidateQueries({ queryKey: ["customer-oversight", "orders-for-day", openDayId] });
    },
  ...
  return (
    <ListDetailLayout
      header={<PageHeader title="בשם לקוח" subtitle="צפייה ועריכה של הזמנות היום בשם כל לקוח, ושליחת תזכורות." />}
      list={
              ...
              {customersQuery.data?.map((row) => {
                ...
                return (
                  <li key={row.id}>
                    ...
                    >
                      <span>{row.name}</span>
                      <span className="text-xs text-ink-muted">{order ? STATUS_LABEL[order.status] : "אין הזמנה"}</span>
                    </button>
                  </li>
                );
              })}
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <p className="text-sm text-ink-muted">
                  {selectedOrder ? (
                    <>
                      ...
                      {selectedOrder.submitted_at && ` · נשלח ב-${new Date(selectedOrder.submitted_at).toLocaleString("he-IL")}`}
              ...
              <Button
                ...
                disabled={!selectedOrder || selectedOrder.status === "submitted" || reminderMutation.isPending}
              >
